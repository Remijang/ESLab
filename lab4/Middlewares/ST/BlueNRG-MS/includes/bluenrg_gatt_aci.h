...
/**
 * @brief Add a service to the GATT Server. When a service is created in the server, the
 * Host needs to reserve the handle ranges for this service using max_attr_records
 * parameter. This parameter specifies the maximum number of attribute records that can be
 * added to this service (including the service attribute, include attribute,
 * characteristic attribute, characteristic value attribute and characteristic descriptor
 * attribute). Handle of the created service is returned.
 * @note  Service declaration is taken from the service pool. The attributes for
 * characteristics and descriptors are allocated from the attribute pool.
 ...
 * @param max_attr_records Maximum number of attribute records that can be added to this
 * service (including the service declaration itself)
 * @param[out] serviceHandle Handle of the Service. When this service is added to the
 * service, a handle is allocated by the server to this service. Server also allocates a
 * range of handles for this service from serviceHandle to <serviceHandle +
 * max_attr_records>.
 * @return Value indicating success or error code.
 */
...
/**
 * @brief Include a service given by included_start_handle and included_end_handle to
 * another service given by service_handle. Attribute server creates an INCLUDE definition
 ...
 * @param included_start_handle Start Handle of the service which has to be included in
 * service
 * @param included_end_handle End Handle of the service which has to be included in
 * service
 * @param included_uuid_type Type of UUID for included service (16-bit or 128-bit). See
 * @ref Well-Known_UUIDs "Well-Known UUIDs".
 ...
 * @param charUuidType Type of characteristic UUID (16-bit or 128-bit). See @ref
 ...
 * @param charProperties Bitwise OR values of Characteristic Properties (defined in Volume
 * 3, Section 3.3.3.1 of Bluetooth Specification 4.0). See @ref Char_properties
 * "Characteristic properties".
 * @param secPermissions Security permissions for the added characteristic. See @ref
 ...
 * @param gattEvtMask Bit mask that enables events that will be sent to the application by
 * the GATT server on certain ATT requests. See @ref Gatt_Event_Mask "Gatt Event Mask".
 ...
 * @param encryKeySize The minimum encryption key size requirement for this attribute.
 * Valid Range: 7 to 16.
 * @param isVariable If the attribute has a variable length value field (1) or not (0).
 * @param charHandle Handle of the Characteristic that has been added. It is the handle of
 * the characteristic declaration. The attribute that holds the characteristic value is
 * allocated at the next handle, followed by the Client Characteristic Configuration
 * descriptor if the characteristic has @ref CHAR_PROP_NOTIFY or @ref CHAR_PROP_INDICATE
 * 		  properties.
 * @return Value indicating success or error code.
 */
 ...
 *         @arg @ref UUID_TYPE_16
 *         @arg @ref UUID_TYPE_128
 * @param[in] uuid UUID of the Characteristic descriptor. It can be one of the UUID
 * assigned by Bluetooth SIG (Well_known_UUIDs) or a user-defined one.
 ...
 * @param secPermissions Security permissions for the added descriptor. See @ref
 * Security_permissions "Security permissions".
 * 			@arg ATTR_PERMISSION_NONE
 * 			@arg ATTR_PERMISSION_AUTHEN_READ
 * 			@arg ATTR_PERMISSION_AUTHOR_READ
 ...
 * 			@arg ATTR_PERMISSION_AUTHOR_WRITE
 * 			@arg ATTR_PERMISSION_ENCRY_WRITE
 * @param accPermissions Access permissions for the added descriptor. See @ref
 ...
 * @note If notifications (or indications) are enabled on that characteristic, a
 *notification (or indication) will be sent to the client after sending this command to
 *the BlueNRG. The command is queued into the BlueNRG command queue. If the buffer is
 *full, because previous commands could not be still processed, the function will return
 *@ref BLE_STATUS_INSUFFICIENT_RESOURCES. This will happen if notifications (or
 *   	 indications) are enabled and the application calls aci_gatt_update_char_value()
 *at an higher rate than what is allowed by the link. Throughput on BLE link depends on
 *connection interval and connection length parameters (decided by the master, see
 *aci_l2cap_connection_parameter_update_request() for more info on how to suggest new
 *connection parameters from a slave). If the application does not want to lose
 *notifications because BlueNRG buffer becomes full, it has to retry again till the
 *function returns @ref BLE_STATUS_SUCCESS or any other error code.\n Example:\n Here if
 *BlueNRG buffer become full because BlueNRG was not able to send packets for a while,
 *some notifications will be lost.
 *   	 @code
 ...
 *		 }
 *		 @endcode
 *		 Here if BlueNRG buffer become full, the application try again to send the
 *notification.
 *		 @code
 *       struct timer t;
 ...
 * @param charValOffset The offset from which the attribute value has to be updated. If
 *this is set to 0, and the attribute value is of variable length, then the length of the
 *attribute will be set to the charValueLen. If the charValOffset is set to a value
 *greater than 0, then the length of the attribute will be set to the maximum length as
 ...
 * @note  When the ATT MTU exchange procedure is completed, a @ref
 * EVT_BLUE_ATT_EXCHANGE_MTU_RESP event is generated. A @ref
 * EVT_BLUE_GATT_PROCEDURE_COMPLETE event is also generated to indicate the end of the
 * procedure.
 ...
 * @note This command is used to obtain the mapping of attribute handles with their
 * associated types. The responses of the procedure are given through the
 * 		 @ref EVT_BLUE_ATT_FIND_INFORMATION_RESP event. The end of the procedure is
 * indicated by a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given
 * @param start_handle Starting handle of the range of attributes to be discovered on the
 * server
 * @param end_handle Ending handle of the range of attributes to be discovered on the
 * server
 * @return Value indicating success or error code.
 */
 ...
 * 		 The responses of the procedure are given through the @ref
 * EVT_BLUE_ATT_FIND_BY_TYPE_VAL_RESP event. The end of the procedure is indicated by a
 * @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given.
 ...
 * @note  The Read By Type Request is used to obtain the values of attributes where the
 * attribute type is known but the handle is not known. The responses of the procedure are
 * given through the @ref EVT_BLUE_ATT_READ_BY_TYPE_RESP event. The end of the procedure
 * is indicated by a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given.
 * @param start_handle First requested handle number
 * @param end_handle Last requested handle number
 ...
 * @note The Read By Group Type Request is used to obtain the values of grouping
 * attributes where the attribute type is known but the handle is not known. Grouping
 * attributes are defined at GATT layer. The grouping attribute types are: «Primary
 * Service», «Secondary Service» and «Characteristic». The responses of the procedure are
 * given through the @ref EVT_BLUE_ATT_READ_BY_GROUP_TYPE_RESP event. The end of the
 * procedure is indicated by a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE.
 * @param conn_handle Connection handle for which the command is given.
 * @param start_handle First requested handle number
 * @param end_handle Last requested handle number
 * @param uuid_type @arg @ref UUID_TYPE_16
 ...
 * @note  The Prepare Write Request is used to request the server to prepare to write the
 * value of an attribute. The responses of the procedure are given through the @ref
 * EVT_BLUE_ATT_PREPARE_WRITE_RESP event. The end of the procedure is indicated by a @ref
 ...
 * @note The Execute Write Request is used to request the server to write or cancel the
 * write of all the prepared values currently held in the prepare queue from this client.
 *  	 The result of the procedure is given through the @ref
 * EVT_BLUE_ATT_EXEC_WRITE_RESP event. The end of the procedure is indicated by a @ref
 * EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given.
...
/**
 * @brief This command will start the GATT client procedure to discover all primary
 * services on the server.
 * @note  The responses of the procedure are given through the @ref
 * EVT_BLUE_ATT_READ_BY_GROUP_TYPE_RESP event. The end of the procedure is indicated by a
 * @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given.
 * @return Value indicating success or error code.
 */
...
/**
 * @brief Start the procedure to discover the primary services of the specified UUID on
 * the server.
 * @note The responses of the procedure are given through the @ref
 * EVT_BLUE_ATT_FIND_BY_TYPE_VAL_RESP event. The end of the procedure is indicated by a
 * @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 ...
 * @note The responses of the procedure are given through the @ref
 * EVT_BLUE_ATT_READ_BY_TYPE_RESP event. The end of the procedure is indicated by a @ref
 * EVT_BLUE_GATT_PROCEDURE_COMPLETE event.
 * @param conn_handle Connection handle for which the command is given.
 ...
 * @note When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is
 * generated. Before procedure completion the response packets are given through @ref
 * EVT_BLUE_ATT_READ_BY_TYPE_RESP event.
 * @param conn_handle Connection handle for which the command is given
 ...
 * @note  When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event
 ...
 * @note When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is
 * generated. Before procedure completion the response packets are given through @ref
 * EVT_BLUE_ATT_FIND_INFORMATION_RESP event.
 * @param conn_handle Connection handle for which the command is given.
 ...
 * @note  When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event
 *is generated. Before procedure completion the response packet is given through @ref
 *EVT_BLUE_ATT_READ_RESP event.
 * @param conn_handle Connection handle for which the command is given
 ...
 * @note When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is
 *generated. Before procedure completion the response packets are given through @ref
 *EVT_BLUE_ATT_READ_BLOB_RESP event.
 * @param conn_handle Connection handle for which the command is given
 * @param attr_handle Handle of the characteristic to be read
 ...
 * @note This sub-procedure is used to read multiple Characteristic Values from a server
 * when the client knows the Characteristic Value Handles. When the procedure is
 * completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is generated. Before procedure
 ...
 * @note When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is
 * generated. During the procedure, @ref EVT_BLUE_ATT_PREPARE_WRITE_RESP and @ref
 * EVT_BLUE_ATT_EXEC_WRITE_RESP events are raised.
 * @param conn_handle Connection handle for which the command is given
 ...
 * @note When the procedure is completed, a  @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event
 * is generated. During the procedure, @ref EVT_BLUE_ATT_PREPARE_WRITE_RESP and @ref
 * EVT_BLUE_ATT_EXEC_WRITE_RESP events are raised.
 * @param conn_handle Connection handle for which the command is given
 * @param attr_handle Handle of the attribute to be written
 * @param val_offset Offset at which the attribute has to be written
 ...
 * @brief Start the procedure to read a long characteristic value.
 * @note  When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event
 * is generated. Before procedure completion the response packets are given through @ref
 * EVT_BLUE_ATT_READ_BLOB_RESP event.
 * @param conn_handle Connection handle for which the command is given
 ...
 * @note When the procedure is completed, a @ref EVT_BLUE_GATT_PROCEDURE_COMPLETE event is
 * generated. Before procedure completion the response packet is given through @ref
 * EVT_BLUE_ATT_READ_RESP event.
 * @param conn_handle Connection handle for which the command is given
...
/**
 * @brief Start the procedure to write a characteristic value without waiting for any
 ...
 * @note  The procedure i used to write a characteristic value with an authentication
 * signature without waiting for any response from the server. It cannot be used when the
 * link is encrypted.
 * @param conn_handle Connection handle for which the command is given
 * @param attr_handle Handle of the attribute to be written
 ...
 * @note  This command has to be sent when the application receives the event @ref
 ...
 * @note This command has to be sent by the application when it receives the @ref
 * EVT_BLUE_GATT_WRITE_PERMIT_REQ. If the write is allowed, then the status and error code
 * has to be set to 0. If the write is not allowed, then the status has to be set to 1 and
 * the error code has to be set to the error code that has to be passed to the client.
 * @param conn_handle Connection handle for which the command is given
 * @param attr_handle Handle of the attribute that was passed in the event @ref
 * EVT_BLUE_GATT_WRITE_PERMIT_REQ.
 * @param write_status 0x00: The value can be written to the attribute specified by
 * attr_handle\n 0x01: The value cannot be written to the attribute specified by the
 * attr_handle.
 * @param err_code  The error code that has to be passed to the client in case the write
 * has to be rejected.
 * @param att_val_len Length of the value to be written as passed in the event @ref
 * EVT_BLUE_GATT_WRITE_PERMIT_REQ.
 ...
 * @note The application has to send this command when it receives the @ref
 * EVT_BLUE_GATT_READ_PERMIT_REQ or @ref EVT_BLUE_GATT_READ_MULTI_PERMIT_REQ. This command
 * indicates to the stack that the response can be sent to the client. So if the
 * application wishes to update any of the attributes before they are read by the client,
 * it has to update the characteristic values using the aci_gatt_update_char_value and
 * then give this command. The application should perform the required operations within
 * 30 seconds, otherwise the GATT procedure will go to timeout.
 * @param conn_handle Connection handle for which the command is given.
 * @return Value indicating success or error code.
 */
 ...
 * @note  Currently the setting of security permission is allowed only for client
 * configuration descriptor.
 * @param service_handle Handle of the service which contains the attribute whose security
 * 						 permission has to be modified.
 * @param attr_handle	Handle of the attribute whose security permission has to be
 * modified.
 * @param security_permission Security permissions for the descriptor. See @ref
 * Security_permissions "Security permissions".
 * 			@arg ATTR_PERMISSION_NONE
 * 			@arg ATTR_PERMISSION_AUTHEN_READ
 * 			@arg ATTR_PERMISSION_AUTHOR_READ
 ...
 * 				The length will be the minimum between the provided data_len and the
 * actual length of the attribute (in data_len_out_p).
 * @return Value indicating success or error code.
 */
 ...
 * @note This command is a more flexible version of ACI_GATT_UPDATE_CHAR_VALUE to support
 * update of long attribute up to 512 bytes and indicate selectively the generation of
 ...
 * @param update_type Bitmask that controls generation of notifications and indications.
 ...
 * @param char_length Total length of the characteristic value. In case of a variable size
 * characteristic, this field specifies the new length of the characteristic value after
 * the update; in case of fixed length characteristic this field is ignored.
...
/**
 * This event (if enabled, see @ref Gatt_Event_Mask "Gatt Event Mask") is raised to the
 * application by the GATT server when a client modifies any attribute on the server, as
 * consequence of one of the following GATT procedures:
...
typedef __packed struct _evt_gatt_attr_modified_IDB05A1 {
	...
	uint16_t
		offset; /**< Offset from which the write has been performed by the peer device */
...
/**
 * This event is generated in response to a @a Find @a Information @a Request. See
 * aci_att_find_information_req() and Find Information Response in Bluetooth Core v4.0
 * spec. See @ref _evt_att_find_information_resp.
 */
...
typedef __packed struct _evt_att_find_information_resp {
	...
	uint8_t format; /**< The format of the handle_uuid_pair. @arg 1: 16-bit UUIDs @arg 2:
					   128-bit UUIDs */
	/**
 ...
 * Find By Type Value Response in Bluetooth Core v4.0 spec. See @ref
 * _evt_att_find_by_type_val_resp.
 */
...
typedef __packed struct _evt_att_find_by_type_val_resp {
	 ...
	 *  A sequence of handle pairs: [2 octets for Found Attribute Handle, 2 octets for
	 * Group End Handle]
	 */
...
/**
 * This event is generated in response to a @a Read @a By @a Type @a Request. See
 * aci_gatt_find_included_services() and aci_gatt_disc_all_charac_of_serv(). For more info
 * see Read By Type Response in Bluetooth Core v4.0 spec. See @ref
 * _evt_att_read_by_type_resp.
 */
...
/**
 * This event is generated in response to a @a Read @a Request. See
 * aci_gatt_read_charac_val(). For more info see Read Response in Bluetooth Core v4.0
 * spec. See @ref _evt_att_read_resp.
 */
...
/**
 * This event is generated in response to a @a Read @a Blob @a Request. See
 * aci_gatt_read_long_charac_val(). For more info see Read Blob Response in Bluetooth Core
 * v4.0 spec. See @ref _evt_att_read_blob_resp.
 */
 ...
 * For more info see Read Multiple Response in Bluetooth Core v4.0 spec. See @ref
 * _evt_att_read_mult_resp.
 */
...
/**
 * This event is generated in response to a @a Read @a By @a Group @a Type @a Request. See
 * aci_gatt_disc_all_prim_services(). For more info see Read By Group type Response in
 * Bluetooth Core v4.0 spec. See @ref _evt_att_read_by_group_resp.
 */
 ...
 * For more info see Prepare Write Response in Bluetooth Core v4.0 spec. See @ref
 * _evt_att_prepare_write_resp.
 */
...
typedef __packed struct _evt_att_prepare_write_resp {
	...
	uint8_t
		part_attr_value[VARIABLE_SIZE]; /**< The value of the attribute to be written. */
} PACKED evt_att_prepare_write_resp;

/**
 * This event is generated in response to an @a Execute @a Write @a Request.
 * For more info see Execute Write Response in Bluetooth Core v4.0 spec. See @ref
 * _evt_att_exec_write_resp.
 */
 ...
 * For more info see Handle Value Indication in Bluetooth Core v4.0 spec. See @ref
 * _evt_gatt_indication.
 */
 ...
 * For more info see Handle Value Notification in Bluetooth Core v4.0 spec. See @ref
 * _evt_gatt_notification.
 */
 ...
 * successfully. See @ref _evt_gatt_procedure_complete.
 */
#define EVT_BLUE_GATT_PROCEDURE_COMPLETE (0x0C10)
typedef __packed struct _evt_gatt_procedure_complete {
	uint16_t conn_handle; /**< The connection handle on which the GATT procedure has
							 completed */
	uint8_t data_length;  /**< Length of error_code field (always 1). */
	/**
	 * Indicates whether the procedure completed with error (BLE_STATUS_FAILED) or was
	 * successful (BLE_STATUS_SUCCESS).
	 */
	uint8_t error_code;
 ...
 * This does not mean that the procedure ended with an error, but this error event is part
 * of the procedure itself. See @ref _evt_gatt_error_resp.
 */
...
typedef __packed struct _evt_gatt_error_resp {
	...
	uint8_t error_code;	  /**< The reason why the request has generated an error response.
							 See Error Response in Bluetooth Core v4.0 spec.  */
} PACKED evt_gatt_error_resp;

/**
 * This event can be generated during a "Discover Characteristics By UUID" procedure or a
 * "Read using Characteristic UUID" procedure. The attribute value will be a service
 * declaration as defined in Bluetooth Core v4.0 spec (vol.3, Part G, ch. 3.3.1), when a
 * "Discover Characteristics By UUID" has been started. It will be the value of the
 * Characteristic if a "Read using Characteristic UUID" has been performed. See @ref
 * _evt_gatt_disc_read_char_by_uuid_resp.
 */
...
typedef __packed struct _evt_gatt_disc_read_char_by_uuid_resp {
	...
	uint16_t attr_handle;	   /**< The handle of the attribute. */
	/**
	 * The attribute value will be a service declaration as defined in Bluetooth Core v4.0
	 * spec (vol.3, Part G, ch. 3.3.1), when a "Discover Characteristics By UUID" has been
	 * started. It will be the value of the Characteristic if a "Read using Characteristic
	 * UUID" has been performed.
	 */
	uint8_t attr_value[VARIABLE_SIZE];
} PACKED evt_gatt_disc_read_char_by_uuid_resp;

/**
 * This event is given to the application when a write request, write command or signed
 * write command is received by the server from the client. This event will be given to
 * the application only if the event bit for this event generation is set when the
 * characteristic was added. When this event is received, the application has to check
 * whether the value being requested for write is allowed to be written and respond with
 * the command aci_gatt_write_response(). If the write is rejected by the application,
 * then the value of the attribute will not be modified. In case of a write request, an
 * error response will be sent to the client, with the error code as specified by the
 * application. In case of write/signed write commands, no response is sent to the client
 * but the attribute is not modified. See @ref evt_gatt_write_permit_req.
 */
#define EVT_BLUE_GATT_WRITE_PERMIT_REQ (0x0C13)
typedef __packed struct _evt_gatt_write_permit_req {
	uint16_t conn_handle; /**< Handle of the connection on which there was the request to
							 write the attribute. */
	uint16_t attr_handle; /**< The handle of the attribute for which the write request has
...
/**
 * This event is given to the application when a read request or read blob request is
 * received by the server from the client. This event will be given to the application
 * only if the event bit for this event generation is set when the characteristic was
 * added. On receiving this event, the application can update the value of the handle if
 * it desires and when done it has to use the aci_gatt_allow_read() command to indicate to
 * the stack that it can send the response to the client. See @ref
...
typedef __packed struct _evt_gatt_read_permit_req {
	uint16_t conn_handle; /**< Handle of the connection on which there was the request to
							 read the attribute. */
	uint16_t attr_handle; /**< The handle of the attribute for which the read request has
							 been made by the client */
...
/**
 * This event is given to the application when a read multiple request or read by type
 * request is received by the server from the client. This event will be given to the
 * application only if the event bit for this event generation is set when the
 * characteristic was added. On receiving this event, the application can update the
 * values of the handles if it desires and when done it has to send the
 * aci_gatt_allow_read command to indicate to the stack that it can send the response to
 * the client. See @ref evt_gatt_read_multi_permit_req.
 *
 */
...
typedef __packed struct _evt_gatt_read_multi_permit_req {
	uint16_t conn_handle; /**< Handle of the connection on which there was the request to
							 read the attribute. */
	uint8_t data_length;  /**< Length of data field. */
	uint8_t data[VARIABLE_SIZE]; /**< The handles of the attributes that have been
									requested by the client for a read. */
} PACKED evt_gatt_read_multi_permit_req;

/**
 * This event is raised when the number of available TX buffers is above a threshold TH
 * (TH = 2). The event will be given only if a previous ACI command returned with
 * BLE_STATUS_INSUFFICIENT_RESOURCES. On receiving this event, the application can
 * continue to send notifications by calling aci_gatt_update_char_value(). See @ref
 * evt_gatt_tx_pool_vailable.
 *
 */
...
/**
 * This event is raised on the server when the client confirms the reception of an
 * indication.
 */
 ...
 * only if the event bit for this event generation is set when the characteristic was
 * added. When this event is received, the application has to check whether the value
 * being requested for write is allowed to be written and respond with the command
 * aci_gatt_write_response(). Based on the response from the application, the attribute
 * value will be modified by the stack. If the write is rejected by the application, then
 * the value of the attribute will not be modified and an error response will be sent to
 * the client, with the error code as specified by the application. See @ref
