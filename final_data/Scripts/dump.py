...
def format_1d_array(tensor, indent=4):
    ...
    line = " " * indent + ", ".join(
        f"{v:d}" if type(v) is int else f"{v}" for v in values
    )
    return line
...
def format_2d_array(tensor, indent=1):
    ...
    for i, row in enumerate(rows):
        row_str = ", ".join(f"{v:d}" if type(v) is int else f"{v}" for v in row)
        if i < len(rows) - 1:
...
def main():
    ...
    # Print Alpha
    print(
        f"const float rnn_alpha = {alpha_val:.4f};\t // alpha is used to make rnn able to convert the unit between cm and z-axis unit"
    )
    ...
    if args.quant:
        ...
        if w_ih_layers is not None:
            print(
                f"const int16_t weight_ih_layers[LAYER_NUM - 1][3 * HIDDEN_SIZE][HIDDEN_SIZE] = {{"
            )
            print(format_3d_array(q_w_ih_layers))
            print("};\n")

        # bias_ih
        print(f"const int16_t bias_ih[LAYER_NUM][3 * HIDDEN_SIZE] = {{")
        print(format_2d_array(q_b_ih))
        print("};\n")

        # weight_hh
        print(f"const int16_t weight_hh[LAYER_NUM][3 * HIDDEN_SIZE][HIDDEN_SIZE] = {{")
        print(format_3d_array(q_w_hh))
        print("};\n")

        # bias_hh
        print(f"const int16_t bias_hh[LAYER_NUM][3 * HIDDEN_SIZE] = {{")
        ...
        # bias_output
        print(
            f"const int16_t bias_output[{output_size}] = {{{format_1d_array(q_b_out, indent=0)}}};"
        )
    else:
        print(f"const float weight_ih[3 * HIDDEN_SIZE][{input_size}] = {{")
        print(format_2d_array(w_ih_0))
        print("};\n")

        # weight_ih_layers
        if w_ih_layers is not None:
            print(
                f"const float weight_ih_layers[LAYER_NUM - 1][3 * HIDDEN_SIZE][HIDDEN_SIZE] = {{"
            )
            print(format_3d_array(w_ih_layers))
            print("};\n")

        # bias_ih
        print(f"const float bias_ih[LAYER_NUM][3 * HIDDEN_SIZE] = {{")
        print(format_2d_array(b_ih))
        print("};\n")

        # weight_hh
        print(f"const float weight_hh[LAYER_NUM][3 * HIDDEN_SIZE][HIDDEN_SIZE] = {{")
        print(format_3d_array(w_hh))
        print("};\n")

        # bias_hh
        print(f"const float bias_hh[LAYER_NUM][3 * HIDDEN_SIZE] = {{")
        print(format_2d_array(b_hh))
        print("};\n")

        # weight_output
        print(f"const float weight_output[{output_size}][HIDDEN_SIZE] = {{")
        print(format_2d_array(w_out))
        print("};\n")

        # bias_output
        print(
            f"const float bias_output[{output_size}] = {{{format_1d_array(b_out, indent=0)}}};"
        )
