...
/** @defgroup B_L475E_IOT01A1_LOW_LEVEL_Private_Functions B_L475E_IOT01A1 LOW LEVEL
 * Private Functions
 * @{
 */

/** @defgroup B_L475E_IOT01A1_BUS_Exported_Functions B_L475E_IOT01A1_BUS Exported
 * Functions
 * @{
 */
...
int32_t BSP_SPI3_SendRecv(uint8_t* pTxData, uint8_t* pRxData, uint16_t Length) {
	...
	if (HAL_SPI_TransmitReceive(
			&hspi3, pTxData, pRxData, Length, BUS_SPI3_POLL_TIMEOUT
		) != HAL_OK) {
		ret = BSP_ERROR_UNKNOWN_FAILURE;
	}
	return ret;
...
int32_t BSP_SPI3_RegisterDefaultMspCallbacks(void) {
	...
	if (HAL_SPI_RegisterCallback(&hspi3, HAL_SPI_MSPDEINIT_CB_ID, SPI3_MspDeInit) !=
		HAL_OK) {
		return BSP_ERROR_PERIPH_FAILURE;
	}
	IsSPI3MspCbValid = 1;
...
int32_t BSP_SPI3_RegisterMspCallbacks(BSP_SPI_Cb_t* Callbacks) {
	...
	/* Register MspInit Callback */
	if (HAL_SPI_RegisterCallback(&hspi3, HAL_SPI_MSPINIT_CB_ID, Callbacks->pMspInitCb) !=
		HAL_OK) {
		return BSP_ERROR_PERIPH_FAILURE;
	}

	/* Register MspDeInit Callback */
	if (HAL_SPI_RegisterCallback(
			&hspi3, HAL_SPI_MSPDEINIT_CB_ID, Callbacks->pMspDeInitCb
		) != HAL_OK) {
		return BSP_ERROR_PERIPH_FAILURE;
	}
