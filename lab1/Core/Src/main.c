...
int main(void) {
	...
	semaphore = osSemaphoreNew(1U, 1U, &binarySem_attributes);
	...
	/* Create the queue(s) */
	/* creation of Queue */
	queue = osMessageQueueNew(MSG_COUNT, sizeof(uint32_t), &queue_attributes);

	/* Create the thread(s) */
	/* creation of Task1 */
	tid1 = osThreadNew(Task1_Func, NULL, &task1_attributes);

	/* creation of Task2 */
	tid2 = osThreadNew(Task2_Func, NULL, &task2_attributes);
...
void Task1_Func(void *argument) {
	...
	while (1) {
		(void)osMessageQueueGet(queue, &diff, NULL, osWaitForever);
