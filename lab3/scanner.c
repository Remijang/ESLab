...
static void usage(char* argv[]) {
	printf(
		"%s <device_address> <notification_characteristic_uuid> "
		"[<write_characteristic_uuid> <write_characteristic_data>]\n",
		argv[0]);
}
...
static void on_device_connect(gattlib_adapter_t* adapter,
	...
	if (m_argument.gatt_write_data != 0) {
		ret = gattlib_write_char_by_uuid(
			connection, &m_argument.gatt_write_uuid, &m_argument.gatt_write_data, 1);
...
static void ble_discovered_device(gattlib_adapter_t* adapter,
	...
	if (ret == 0) {
		GATTLIB_LOG(GATTLIB_INFO,
					"Found bluetooth device '%s' with RSSI:%d",
	...
	ret = gattlib_connect(
		adapter, addr, GATTLIB_CONNECTION_OPTIONS_NONE, on_device_connect, NULL);
	if (ret != GATTLIB_SUCCESS) {
		GATTLIB_LOG(
			GATTLIB_ERROR, "Failed to connect to the bluetooth device '%s'", addr);
	}
}
...
static void* ble_task(void* arg) {
	...
	ret = gattlib_adapter_scan_enable(
		adapter, ble_discovered_device, BLE_SCAN_TIMEOUT, addr);
	if (ret) {
...
int main(int argc, char* argv[]) {
	...
	if (gattlib_string_to_uuid(
			argv[2], strlen(argv[2]) + 1, &m_argument.gatt_notification_uuid) < 0) {
		usage(argv);
		return 1;
	}
	...
	if (argc == 5) {
		if (gattlib_string_to_uuid(
				argv[3], strlen(argv[3]) + 1, &m_argument.gatt_write_uuid) < 0) {
			usage(argv);
			return 1;
		}
